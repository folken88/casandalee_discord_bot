...
**Symptoms**: `/memory` returns nothing.

(Note: `/memory` posts a random past-life **## Timeline Quote** on demand. The daily Recollection no longer uses these quotes — it uses an OpenAI-generated comment instead — so this section applies to `/memory` only.)

**Solutions**:

1. **Restart Discord** — Slash commands and replies can be cached; fully quit and reopen the Discord client.
2. **Ensure timeline quotes exist** — `/memory` uses only the **## Timeline Quote** from each personality. Generate them with:
   ```bash
...
### Daily Recollection Not Appearing / Cass Is Silent in the Morning

**Symptoms**: No "📜 Casandalee's Recollection" embed appears in the morning, even though the bot is online and responding to commands.

**How it works**: Once a day, at a **random time between 6:00 and 8:00 AM (America/Chicago)**, the bot posts one Recollection embed (a campaign event + Cass's comment) to the General channel. A fresh drop time is rolled each morning and stored in `data/cache/daily-state.json` (`recollectionTargetMin`); the post fires once the clock passes it, and `dailyRecollectionDate` prevents a second post that day.

**Cause A — Already posted today**: If `dailyRecollectionDate` in `daily-state.json` equals today's date, it won't post again. Expected.

**Cause B — No in-window events**: If the timeline has no events in 4700–4717.06 AR, the post is skipped (won't happen with the bundled data).

**Cause C — The comment is missing but the event shows**: The event comes from the timeline; the comment is generated by OpenAI. If no LLM provider is configured (or the call fails), the embed still posts with a static fallback line. Check that `OPENAI_API_KEY` (or another provider key) is set.

**Diagnosis**:
```bash
docker logs cass-discord-bot | grep -E "Recollection|drop|provider"

# Force one immediately (admin): use the /daily-history trigger command in Discord,
# or restart — _tick() runs on startup and catches up if past the rolled time:
docker restart cass-discord-bot
```

**Note — manual controls**:
- `/daily-history trigger` (admin) posts a Recollection to General now.
- `/daily-history today` previews one just for you.
- The old "Today in Golarion History" (real-life-date matching) and standalone random quotes were **removed** — the Recollection replaced them.
